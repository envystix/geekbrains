...
class Currency {
     ...
     * @returns {string} Выводит значение цены с символом валюты для подстановки в HTML-код
     */
    getPrice(price) {
        if (this.position == 1) {
...
/** Генерация избранных товаров
 * @param {array} array - Массив избранных товаров
 * @param {string} sortType - Тип сортировки. Может принимать значения: featured или new
 * @param {string} order - Направление сортировки. Может принимать значения: ascending или descending
 * @param {number} quantity - Количество карточек для вывода на страницу
 */
function sortProductCards(array = products, sortType = 'featured', order = 'ascending', quantity = 6) {
    let productCardsBlock = '';
    let productCards = array;
    if (sortType == 'featured') {
        productCardsBlock = document.querySelector('.featured-cards');
        if (order == 'ascending') {
            productCards.sort((a, b) => a.salesCounter - b.salesCounter);
        } else if (order == 'descending') {
            productCards.sort((a, b) => b.salesCounter - a.salesCounter);
        } else {
            console.log('Неверное значение сортировки');
        }
    } else if (sortType == 'new') {
        productCardsBlock = document.querySelector('.new-cards');
        productCards.sort((a, b) => a.publicationDate - b.publicationDate);
        if (order == 'ascending') {
            productCards.sort((a, b) => a.publicationDate - b.publicationDate);
        } else if (order == 'descending') {
            productCards.sort((a, b) => b.publicationDate - a.publicationDate);
        } else {
            console.log('Неверное значение сортировки');
        }
    } else {
    ...
    if (productCardsBlock != null) {
        for (let i = 0; i < quantity; i++) {
            productCardsBlock.insertAdjacentHTML('beforeend', `
                <div class="main__featured_cards-card_${productCards[i].id} product-card product-card-hover">
                    <div class="main__featured_cards-card_${productCards[i].id}-img-wrapper product-card-img-wrapper">
                        <div class="main__featured_cards-card_${productCards[i].id}-img-hover product-card-hover-img">
                            <form action="cart.html">
                            <button class="main__featured_cards-card_${productCards[i].id}-img-hover-btn product-card-hover-btn">
                        ...
                        <img src="${productCards[i].picture}" alt="${productCards[i].title}" class="main__featured_cards-card_${productCards[i].id}-img product-card-img">
                    </div>
                    <a class="main__featured_cards-link product-card-link" href="product.html">
                        <div class="main__featured_cards-card_${productCards[i].id}-description product-card-text">
                            <h3 class="main__featured_cards-card_${productCards[i].id}-description-title product-card-title">${productCards[i].title}</h3>
                            <p class="main__featured_cards-card_${productCards[i].id}-description-subtitle product-card-description">${productCards[i].description}</p>
                            <p class="main__featured_cards-card_${productCards[i].id}-description-cost product-card-cost">${productCards[i].priceHtml}</p>
                        </div>
                    </a>
                </div>`);
        }
...
sortProductCards(products, 'featured', 'descending', 6);
sortProductCards(products, 'new', 'descending', 9);