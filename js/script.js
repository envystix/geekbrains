...
class Product {
     ...
     */
    constructor(title, description, price, currencyId, pictureName) {
        this.id = productId++;
        this.title = title;
...
const featuredProducts = [
    ...
    Australian arbiter of&nbsp;cool Kym Ellery teams up&nbsp;with Moda Operandi.', 13500, 2, 'card-1.jpg'),
    new Product('Элегантный костюм', 'Known for her sculptural takes on&nbsp;traditional tailoring, \
    Australian arbiter of&nbsp;cool Kym Ellery teams up&nbsp;with Moda Operandi.', 7200, 2, 'card-2.jpg'),
    new Product('Необычное худи', 'Known for her sculptural takes on&nbsp;traditional tailoring, \
    Australian arbiter of&nbsp;cool Kym Ellery teams up&nbsp;with Moda Operandi.', 2300, 2, 'card-3.jpg'),
    new Product('Курортные чинос', 'Known for her sculptural takes on&nbsp;traditional tailoring, \
    Australian arbiter of&nbsp;cool Kym Ellery teams up&nbsp;with Moda Operandi.', 3100, 2, 'card-4.jpg'),
    new Product('Деловой пиджак', 'Known for her sculptural takes on&nbsp;traditional tailoring, \
    Australian arbiter of&nbsp;cool Kym Ellery teams up&nbsp;with Moda Operandi.', 2500, 2, 'card-5.jpg'),
    new Product('Облегченная рубашка', 'Known for her sculptural takes on&nbsp;traditional tailoring, \
    Australian arbiter of&nbsp;cool Kym Ellery teams up&nbsp;with Moda Operandi.', 1700, 2, 'card-6.jpg'),
];

/** Генерация избранных товаров
 * @param {array} array - Массив избранных товаров
 */
function showFeaturedCards(array) {
    let featuredCardsBlock = document.querySelector('.featured-cards');
    array.forEach((item) => {
        featuredCardsBlock.insertAdjacentHTML('beforeend', `
            <div class="main__featured_cards-card_${item.id} product-card product-card-hover">
                <div class="main__featured_cards-card_${item.id}-img-wrapper product-card-img-wrapper">
                    <div class="main__featured_cards-card_${item.id}-img-hover product-card-hover-img">
                        <form action="cart.html">
                        <button class="main__featured_cards-card_${item.id}-img-hover-btn product-card-hover-btn">
                    ...
                    <img src="${item.picture}" alt="${item.title}" class="main__featured_cards-card_${item.id}-img product-card-img">
                </div>
                <a class="main__featured_cards-link product-card-link" href="product.html">
                    <div class="main__featured_cards-card_${item.id}-description product-card-text">
                        <h3 class="main__featured_cards-card_${item.id}-description-title product-card-title">${item.title}</h3>
                        <p class="main__featured_cards-card_${item.id}-description-subtitle product-card-description">${item.description}</p>
                        <p class="main__featured_cards-card_${item.id}-description-cost product-card-cost">${item.priceHtml}</p>
                    </div>
                </a>
            </div>`);
    });
}
showFeaturedCards(featuredProducts);